...
如果你没有 conda 环境，自己电脑上，下载 Anaconda ，https://www.anaconda.com/download，填写邮箱，然后点开邮件里面的连接下载
...
文档：<https://github.com/ChinaJianNengCloud/MemberDocs/blob/main/server/Proxy.md>
...
#创建一个新的名为 myenv 的环境，并安装指定版本的 Python（比如 3.9）。
...
# 以节点 10 为例，当前Windows服务器统一使用7897端口
...
python 你的脚本所在路径.py
python -W ignore 你的脚本.py # 让警告不显示在终端中，用于避免干扰输出结果
```
...
如何移动光标查看历史记录：按下 ctrl+B，松开，然后按 \[，即可进入复制模式，按 q 退出
...
首先在github （<https://github.com/ChinaJianNengCloud>）上面创建远程仓库，点击 new，输入一个英文名称即可，这是一个空的仓库
...
louhao 代表用户名，SIC 代表当前所在的文件夹，有时候最左边会有（base），说明你激活了 conda 的 base 环境
